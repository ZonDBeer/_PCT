...
{
    unique_lock<mutex> lk(lock);						// блокирует крит секцию
    cond_var.wait(lk, []{return ready;});		// пока реди не будет тру, а она будет выполн когда поток выполнися    // если лямбда выражение возвраает тру то мьютекс разблокируется, стадия завершился
